    ...
    describe("parse", function () {
        ...
        context("when given RFC 2397 examples", function () {
            it("should parse the 'brief note' example successfully", function (done) {
                moddataurl.parse("data:,A%20brief%20note", function (err, info) {
                    if (err)
                        return done(err);
                    expect(info).to.deep.equal({
                        mime: "text/plain",
            ...
            it("should parse the 'charset parameter' example successfully", function (done) {
                ...
                moddataurl.parse("data:text/plain;charset=iso-8859-7,%be%d3%be", function (err, info) {
                    if (err)
                        return done(err);
                    expect(info).to.deep.equal({
                        mime: "text/plain",
            ...
            it("should parse the 'Larry' example successfully", function (done) {
                ...
                moddataurl.parse(larry, function (err, info) {
                    if (err)
                        return done(err);
                    expect(info).to.deep.equal({
                        mime: 'image/gif',
            ...
            it("should parse the 'application/vnd-xxx-query' example successfully", function (done) {
                ...
                moddataurl.parse(vnd, function (err, info) {
                    if (err)
                        return done(err);
                    expect(info).to.deep.equal({
        ...
        describe("type/subtype and charset", function () {
            context("when given a minimal dataurl without charset and without type/subtype", function () {
                it("should parse it successfully and default to text/plain;charset=US-ASCII", function (done) {
                    moddataurl.parse("data:,", function (err, info) {
                        if (err)
                            return done(err);
                        expect(info).to.deep.equal({
                            mime: "text/plain",
            ...
            context("when given a dataurl with charset but no type/subtype", function () {
                it("should parse it successfully and default to text/plain", function (done) {
                    moddataurl.parse("data:;charset=utf-8,", function (err, info) {
                        if (err)
                            return done(err);
                        expect(info).to.deep.equal({
                            mime: "text/plain",
            ...
            context("when given a dataurl with type/subtype and charset specified", function () {
                it("should parse it successfully", function (done) {
                    moddataurl.parse("data:text/plain;charset=ISO-8859-1,", function (err, info) {
                        if (err)
                            return done(err);
                        expect(info).to.deep.equal({
                            mime: "text/plain",
        ...
        describe("parameters", function () {
            context("when given a dataurl with several parameters", function () {
                it("should parse it successfully", function (done) {
                    moddataurl.parse("data:text/plain;charset=cp866;foo=bar;answer=42,", function (err, info) {
                        if (err)
                            return done(err);
                        expect(info).to.deep.equal({
                            mime: "text/plain",
            ...
            context("when given an URL encoded key parameter", function () {
                it("should parse it successfully", function (done) {
                    moddataurl.parse("data:text/plain;A%20brief%20note=hello,", function (err, info) {
                        if (err)
                            return done(err);
                        expect(info).to.deep.equal({
                            mime: "text/plain",
            ...
            context("when given an URL encoded value parameter", function () {
                it("should parse it successfully", function (done) {
                    moddataurl.parse("data:text/plain;hello=A%20brief%20note,", function (err, info) {
                        if (err)
                            return done(err);
                        expect(info).to.deep.equal({
                            mime: "text/plain",
        ...
        describe("base64 encoding", function () {
            context("when given a dataurl with base64 encoded text with type/subtype specified", function () {
                it("should parse it successfully", function (done) {
                    moddataurl.parse("data:text/plain;base64,SGVsbG8gV29ybGQ=", function (err, info) {
                        if (err)
                            return done(err);
                        expect(info).to.deep.equal({
                            mime: "text/plain",
            ...
            context("when given a minimal dataurl with a base64 encoded image", function () {
                it("should parse it successfully", function (done) {
                    ...
                    moddataurl.parse("data:image/gif;base64," + data, function (err, info) {
                        if (err)
                            return done(err);
                        expect(info).to.deep.equal({
                            mime: "image/gif",
    ...
    describe("compose", function () {
        ...
        context("when data is invalid", function () {
            context("when the given data is not a Buffer", function () {
                it("should callback an 'unexpected type for info.data' error", function (done) {
                    var info = {
                        data: new Date(),
                    };
                    moddataurl.compose(info, function (err, dataurl) {
                        expect(err).to.be.an.instanceof(TypeError);
                        expect(err.message).to.equal("expected info.data to be a Buffer");
                        return done();
                    });
                });
            });
